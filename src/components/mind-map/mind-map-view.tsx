...
function calculateNodePositions(messages: Message[], multiModelGroups: Map<string, string[]>) {
  ...
  const rootNodes = messages.filter(msg => !msg.parentId);
  let currentY = 0;
  ...
  while (currentLevel.length > 0) {
    ...
    currentLevel.forEach((parent) => {
      const children = messages.filter(msg => msg.parentId === parent.id);
...
export function MindMapView({
  ...
  }, [messages, nodePositions, multiModelGroups, onResumeConversation, onCreateBranch, onCreateMultiModelBranch]);
